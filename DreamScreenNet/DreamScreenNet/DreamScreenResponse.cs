...
namespace DreamScreenNet {
	public abstract class DreamScreenResponse : Message {
		protected DreamScreenResponse(IPAddress target, MessageType type, MessageFlag flag, int @group = 0) : base(target, type, flag, @group) {
		}
		...
		internal static DreamScreenResponse Create(Message message) {
			...
			Console.WriteLine("Created response: " + response.Type.ToString());
			return response;
		}
		...
		public class StateResponse : DreamScreenResponse {
			[JsonProperty]
			public DreamDevice Device { get; private set; }
			internal StateResponse(Message message) : base(message) {
				...
				Console.WriteLine("DEV TYPE: " + type);
				...
				switch (type) {
					...
					case DeviceType.DreamScreen4K:
						...
						Console.WriteLine("Created dreamscreen: " + ds.Name);
						Device = ds;
						break;
				}

				if (Device != null) {
					Console.WriteLine("Device set: " + Device.Name);	
				} else {
					Console.WriteLine("Device is still freaking null??");
				}
