...
namespace DreamScreenNet {
	public partial class DreamScreenClient {
		...
		private int _subscriptionGroup;
		...
		public void StartSubscribing(int subGroup) {
			_subscribing = true;
			_subscriptionGroup = subGroup;
		}
		...
		private async Task AnnounceSubscription(int @group, bool isRequest = false) {
			...
			if (isRequest) flag = MessageFlag.SystemMessage;
			var msg = new Message(target, MessageType.Subscribe, flag, group);
			if (flag == MessageFlag.SystemMessage) msg.Payload = new Payload(new object[] {(byte)0x01});
		...
		public sealed class DeviceSubscriptionEventArgs : EventArgs {
			...
			internal DeviceSubscriptionEventArgs(IPAddress target) => Target = target;
		}
		...
		public sealed class DeviceColorEventArgs : EventArgs {
			...
			internal DeviceColorEventArgs(Color[] colors) => Colors = colors;
		}
	}
}